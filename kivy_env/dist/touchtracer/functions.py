...
def aes(mode, file, password):
    if mode == 'E':
        try:
            ...
            return ":: File '{name}' overwritten.".format(name=str(file))
    else:
        try:
...
def atbash(message):
    ...
    alphaReverse = list(alphaDefault);
...
def caesarS(mode, message, key, final="", alpha=list("ABCDEFGHIJKLMNOPQRSTUVWXYZ")):
    ...
    try:
        ...
        return ":: Key[0] is not int type ::"
    try:
        key[1] = key[1].upper()
    except:
        return ":: Key[1] is not found ::"
    try:
        alpha = caesarS_insert(caesarS_remove(alpha, key[1]), key[0])
    except:
        return ":: Key[1] is not str type ::"
    for index, symbol in enumerate(message):
        if mode == 'E':
...
def hill2x2(mode, message, matrix):
    ...
    if mode == 'E':
        return hill2x2_crypt(getTwoSymbols(message), matrix)
    else:
        return hill2x2_crypt(getTwoSymbols(message), imatrix)
...
def checkErrors(key, alpha):
    if len(key) != 9:
        return ":: len(key) != 9 ::"
    elif getDeter(sliceto(key, alpha)) == 0:
        return ":: det(Key) = 0 ::"
    elif getDeter(sliceto(key, alpha)) % 2 == 0:
        return ":: det(Key) mod 2 = 0 ::"
    elif getDeter(sliceto(key, alpha)) % 13 == 0:
        return ":: det(Key) mod 13 = 0 ::"
    else:
        return None
...
def getAlgbr(det, key, alpha):
    algbrs = [0 for _ in range(9)];
    index = 0
    for string in range(3):
        for column in range(3):
            algbrs[index] = algebratic(string, column, det, key, alpha);
            index += 1
    return algbrs
...
def hill3x3(mode, message, key, final=""):
    ...
    if mode == 'E':
        ...
        deter = getDeter(MatrixKey);
        algbr = getAlgbr(deter, key, alpha)
...
def vernam(mode, message, key):
    ...
    if mode == 'E':
        ...
        for index, symbol in enumerate(message):
            try:
                message[index] = chr((ord(symbol) - int(getNumbers(key)[index]) - 13) % 26 + ord('A'))
            except:
                return ":: Key is not found ::"
        return "".join(message)
