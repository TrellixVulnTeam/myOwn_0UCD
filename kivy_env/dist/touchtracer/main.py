...
class CryptographyApp(App):
    ...
    def getCipher(self, mode):
        mode = mode.text
        mode = instances.get(mode)
        if mode in ['E', 'D']:
            self.result.font_size = 14
            if not self.message.text:
                self.result.text = ":: Message is not found ::"
                return
            if self.toggle[0].state == 'down':
                self.result.text = a1z26(mode, self.message.text)
            elif self.toggle[1].state == 'down':
                self.result.text = adfgvx(mode, self.message.text, self.key.text)
            elif self.toggle[2].state == 'down':
                self.result.text = aes(mode, self.message.text, self.key.text)
            elif self.toggle[3].state == 'down':
                self.result.text = affine(mode, self.message.text, self.key.text)
            elif self.toggle[4].state == 'down':
                self.result.text = atbash(self.message.text)
            elif self.toggle[5].state == 'down':
                self.result.text = bacon(mode, self.message.text)
            elif self.toggle[6].state == 'down':
                self.result.text = book(mode.text, self.message.text, self.key.text)
            elif self.toggle[7].state == 'down':
                self.result.text = caesar(mode, self.message.text, self.key.text)
            elif self.toggle[8].state == 'down':
                self.result.text = caesarS(mode, self.message.text, self.key.text)
            elif self.toggle[9].state == 'down':
                self.result.text = codes(mode, self.message.text)
            elif self.toggle[10].state == 'down':
                self.result.text = couples(self.message.text)
            elif self.toggle[11].state == 'down':
                self.result.text = doubleCifir(mode, self.message.text, self.key.text)
            elif self.toggle[12].state == 'down':
                self.result.text = fence(mode, self.message.text)
            elif self.toggle[13].state == 'down':
                self.result.text = greatcipher(mode, self.message.text)
            elif self.toggle[14].state == 'down':
                self.result.text = gronsfeld(mode, self.message.text, self.key.text)
            elif self.toggle[15].state == 'down':
                self.result.text = hill2x2(mode, self.message.text, self.key.text)
            elif self.toggle[16].state == 'down':
                self.result.text = hill3x3(mode, self.message.text, self.key.text)
            elif self.toggle[17].state == 'down':
                self.result.text = homophonic(mode, self.message.text)
            elif self.toggle[18].state == 'down':
                self.result.text = lattice(self.message.text)
            elif self.toggle[19].state == 'down':
                self.result.text = playfair(mode, self.message.text)
            elif self.toggle[20].state == 'down':
                self.result.text = polibiy(mode, self.message.text)
            elif self.toggle[21].state == 'down':
                self.result.text = ports(mode, self.message.text)
            elif self.toggle[22].state == 'down':
                self.result.text = powVishener(mode, self.message.text, self.key.text)
            elif self.toggle[23].state == 'down':
                self.result.text = psevdo(mode, self.message.text)
            elif self.toggle[24].state == 'down':
                self.result.text = replace(mode, self.message.text)
            elif self.toggle[25].state == 'down':
                self.result.text = rot13(self.message.text)
            elif self.toggle[26].state == 'down':
                self.result.text = rotors(mode, self.message.text)
            elif self.toggle[27].state == 'down':
                self.result.text = rsa(mode, self.message.text, self.key.text)
            elif self.toggle[28].state == 'down':
                self.result.text = syllables(mode, self.message.text)
            elif self.toggle[29].state == 'down':
                self.result.text = tarabar(self.message.text)
            elif self.toggle[30].state == 'down':
                self.result.text = thritemius(mode, self.message.text, self.key.text)
            elif self.toggle[31].state == 'down':
                self.result.text = typex(mode, self.message.text)
            elif self.toggle[32].state == 'down':
                self.result.text = vernam(mode, self.message.text, self.key.text)
            elif self.toggle[33].state == 'down':
                self.result.text = vishener(mode, self.message.text, self.key.text)
